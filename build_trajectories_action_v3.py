...
V3: Dynamic tweet-level S1/S2 only. No S3.
...
S1 = fraction of in-group tweets yesterday with same stance as this action
S2 = fraction of out-group tweets yesterday with same stance as this action
...
Usage:
  python build_trajectories_action_v3.py \
    ...
    --n_workers 32 --min_actions 10 --max_gap_days 60
"""
...
def parse_args():
    ...
    p.add_argument("--out_dir", type=str, default="/data/dchu/covid_masking_irl_action_v3")
...
def _build_traj_chunk(user_ids):
    ...
    for uid in user_ids:
        ...
        for i, (date_str, stance_bin, action) in enumerate(seq):
            ...
            traj.append((date_str, s1, s2, action))
...
def summarize_and_save(trajs, out_dir):
    ...
    for t in trajs:
        ...
        for (dt, s1, s2, a) in t["trajectory"]:
            rows.append((uid, pol, dt, s1, s2, a))
    flat = pd.DataFrame(rows, columns=[
        "user_id", "political_gen", "date",
        "S1_ingroup_stance_agree", "S2_outgroup_stance_agree", "action",
    ])
    ...
    out_rates = np.array([np.mean([s[3] for s in t["trajectory"]]) for t in trajs])
    s_all = np.array([[s[1], s[2]] for t in trajs for s in t["trajectory"]])

    r = []
    r.append("=" * 60)
    r.append("TRAJECTORY SUMMARY (PER-ACTION, DYNAMIC S1/S2 ONLY)")
    r.append("=" * 60)
...
def main():
    ...
    global _G_USER_ACTIONS
    global _G_SORTED_DATES, _G_SORTED_DATES_LIST
    global _G_MIN_ACTIONS, _G_MAX_GAP_DAYS

    args = parse_args()
    print("=" * 60)
    print("IRL TRAJECTORY BUILDER -- COVID Masking (PER-ACTION, S1/S2 ONLY)")
    ...
    print("\n[2/5] Building political label map...")
