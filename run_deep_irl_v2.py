...
V2: S1/S2/S3, per-group median discretization.

Key changes from v1:
  - Per-group median splits: liberal and conservative users get different
    bin boundaries based on their own group's median for all 3 features
  - 8 states (2x2x2) with per-group thresholds
  - Trajectory tuples: (date, s1, s2, s3, action)

Usage:
  python run_deep_irl_v2.py --traj_dir /data/dchu/covid_masking_irl_action_v2 \
                            --n_bins 2 --epochs 3000 --lr 0.01 --n_jobs 32
"""
...
# ====================================================================
# DISCRETIZATION (per-group median splits, S1/S2/S3)
# ====================================================================
def discretize(trajs, n_bins=2, custom_edges=None):
    """
    Discretize S1/S2/S3 into bins using PER-GROUP medians.
    Each political group gets its own median threshold for all 3 features.
    Trajectory tuples: (date, s1, s2, s3, action).
    """
    n_features = 3
    ...
    group_vals = {0: [[] for _ in range(n_features)],
                  1: [[] for _ in range(n_features)]}
    for t in trajs:
        pol = t["political_gen"]
        for step in t["trajectory"]:
            for f in range(n_features):
                group_vals[pol][f].append(step[1 + f])

    # Compute per-group edges
    if custom_edges is not None:
        group_edges = {0: [np.array(e) for e in custom_edges],
                       1: [np.array(e) for e in custom_edges]}
    else:
        group_edges = {}
        for pol in [0, 1]:
            if not group_vals[pol][0]:  # no data for this group
                continue
            edges = []
            for f in range(n_features):
    ...
    for pol, lab in [(0, "Liberal"), (1, "Conservative")]:
        if pol not in group_edges:
            continue
        print(f"  {lab}:")
        actual_bins = [len(e) - 1 for e in group_edges[pol]]
        for f in range(n_features):
            print(f"    {feature_names[f]} edges: {np.round(group_edges[pol][f], 3)} ({actual_bins[f]} bins)")

    # State space: 2 x 2 x 2 = 8 states (same size for both groups)
    actual_bins = [n_bins, n_bins, n_bins]
    ...
    # Feature map: one-hot identity
    feature_map = np.eye(n_states)

    # Build discretized trajectories
    disc_trajs = []
    meta = []
    ...
    for t in trajs:
        ...
        dt = []
        for step in traj:
            feats = [step[1], step[2], step[3]]
            s = to_state(feats, pol)
            action = step[-1]
            dt.append([s, action])
