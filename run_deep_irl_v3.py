...
V3: S1/S2 only (no S3), per-group median discretization.

Key changes from v1:
  - 2 features (S1, S2) instead of 3 (S1, S2, S3)
  - Per-group median splits: liberal and conservative users get different
    bin boundaries based on their own group's median
  - 4 states (2x2) instead of 8 (2x2x2)
  - Trajectory tuples: (date, s1, s2, action)

Usage:
  python run_deep_irl_v3.py --traj_dir /data/dchu/covid_masking_irl_action_v3 \
...
# ====================================================================
# DISCRETIZATION (per-group median splits, S1/S2 only)
# ====================================================================
def discretize(trajs, n_bins=2, custom_edges=None):
    """
    Discretize S1/S2 into bins using PER-GROUP medians.
    Each political group gets its own median threshold for S1 and S2.
    Trajectory tuples: (date, s1, s2, action).
    """
    n_features = 2
    feature_names = ["S1", "S2"]
    ...
    # State space: 2 x 2 = 4 states (same size for both groups)
    actual_bins = [n_bins, n_bins]
